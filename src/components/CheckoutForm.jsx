...
const CheckoutForm = ({ cart, user, total, itemsPrice, taxPrice, shippingPrice, shippingAddress, clearCart, onDisplaySuccess, validateForm }) => {
    ...
    const [activeTab, setActiveTab] = useState('gateway'); // 'gateway', 'card', 'upi', 'cod', 'wallet'
    ...
    const handleWalletPayment = async (e) => {
        ...
        try {
            // Check frontend balance first (optional but good UX)
            // But we can trust backend error.

            const createdOrder = await createOrder('WALLET');
            // If success, it returns order. Backend throws error if insufficient funds.
    ...
    const handleGatewayPayment = async (e) => {
        ...
        try {
            ...
            if (paymentData.gateway === 'razorpay') {
                ...
                // Prioritize User Profile (Verified) Phone Number over Shipping Address Phone
                const contactNumber = user.phoneNumber || shippingAddress.phoneNumber || '';
                const cleanContact = contactNumber.replace(/\D/g, ''); // Ensure digits only

                console.log("Initiating Razorpay with Contact:", cleanContact);
                    ...
                    handler: async function (response) {
                        try {
                            ...
                            if (verifyResponse.data.success) {
                                await markOrderPaid(createdOrder._id, response.razorpay_payment_id, 'Razorpay');
                            } else {
                ...
                rzp1.on('payment.failed', function (response) {
                    ...
                    api.put(`/orders/${createdOrder._id}/cancel`, { reason: 'Payment Failed: ' + response.error.description }).catch(console.error);
                });
                rzp1.open();

            } else if (paymentData.gateway === 'cashfree') {
                // ... legacy logic or removed
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-wrap p-1 bg-gray-100 dark:bg-slate-700 rounded-xl gap-1">
                <button
                    ...
                    className={`flex-1 min-w-[80px] flex items-center justify-center gap-2 py-2.5 text-sm font-semibold rounded-lg transition-all ${activeTab === 'wallet' ? 'bg-white dark:bg-slate-600 shadow-sm text-purple-600 dark:text-purple-400' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}
                >
            ...
            {activeTab === 'gateway' ? (
                <form onSubmit={handleGatewayPayment} className="space-y-6 fade-in">
                    <div className="p-6 border-2 border-dashed border-green-200 dark:border-green-800 rounded-xl bg-green-50/30 dark:bg-green-900/20">
                        <div className="flex flex-col items-center text-center">
                            ...
                            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">Pay with {activeGateway === 'razorpay' ? 'Razorpay' : (activeGateway === 'cashfree' ? 'Any Method' : 'Instamojo')}</h3>
                ...
                <form onSubmit={handleWalletPayment} className="space-y-6 fade-in">
                    <div className="flex flex-col items-center justify-center p-6 border-2 border-dashed border-purple-200 dark:border-purple-800 rounded-xl bg-purple-50/30 dark:bg-purple-900/20">
                        <div className="bg-white dark:bg-slate-800 p-4 rounded-full shadow-sm mb-4">
                            <Wallet className="h-12 w-12 text-purple-600 dark:text-purple-400" />
                        </div>
                        <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">Pay with Wallet</h3>
                        <p className="text-sm text-slate-600 dark:text-gray-400 text-center max-w-xs">
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full py-4 bg-purple-600 text-white rounded-xl font-bold hover:bg-purple-700 transition-all shadow-lg hover:shadow-xl active:scale-95 flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
                    >
                        {loading ? 'Processing...' : `Pay ₹${finalTotal}`}
                    </button>
                </form>
