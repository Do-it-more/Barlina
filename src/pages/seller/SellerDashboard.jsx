...
const SellerDashboard = () => {
    ...
    useEffect(() => {
        const fetchStats = async () => {
            try {
                ...
                // Fetch earnings data for approved sellers
    ...
    const fetchEarnings = async () => {
        ...
        try {
            const { data } = await api.get('/settlements/earnings-summary');
            setEarnings(data);
        } catch (error) {
        ...
        <div className="space-y-6">
                    ...
                    <div className="flex items-start gap-4">
                        <div className={`p-3 rounded-xl ${isPending ? 'bg-amber-500/20' :
                            isRejected ? 'bg-red-500/20' :
                                isSuspended ? 'bg-orange-500/20' : 'bg-blue-500/20'
                            }`}>
                        ...
                        <div className="flex-1">
                            <h3 className={`font-bold ${isPending ? 'text-amber-400' :
                                isRejected ? 'text-red-400' :
                                    isSuspended ? 'text-orange-400' : 'text-blue-400'
                                }`}>
                            ...
                            <p className="text-gray-300 text-sm mt-1">
                                {isPending && 'Your seller application is being reviewed by our team. This typically takes 2-3 business days.'}
                                {isRejected && 'Unfortunately, your application was rejected. Please check the rejection reason below and reapply.'}
                            ...
                            {isDraft && (
                                <Link to="/seller/onboarding" className="inline-flex items-center gap-2 mt-3 px-4 py-2 bg-violet-600 hover:bg-violet-700 text-white text-sm font-medium rounded-lg transition-colors">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-5 border border-slate-700/50 hover:border-violet-500/30 transition-all group">
                    ...
                    <p className="text-2xl font-bold text-white mt-1">{earnings?.summary?.totalOrders || stats?.ordersCount || 0}</p>
                </div>
                ...
                <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-5 border border-slate-700/50 hover:border-violet-500/30 transition-all group">
                    ...
                    <p className="text-2xl font-bold text-white mt-1">{formatCurrency(earnings?.summary?.grossRevenue || stats?.revenue)}</p>
                </div>
                ...
                <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-5 border border-slate-700/50 hover:border-violet-500/30 transition-all group">
                    ...
                    <p className="text-2xl font-bold text-white mt-1">{earnings?.summary?.commissionRate || stats?.commissionRate || 10}%</p>
            ...
            {isApproved && (
                <div className="space-y-4">
                    <div className="flex items-center justify-between">
                        ...
                        <Link to="/seller/finance" className="text-sm text-violet-400 hover:text-violet-300 flex items-center gap-1">
                            View Detailed Finance <ChevronRight className="h-4 w-4" />
                        </Link>
                    </div>
                    ...
                    {earningsLoading ? (
                        ...
                        <>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                {/* Net Profit */}
                                <div className="bg-gradient-to-br from-emerald-500/20 to-green-500/10 rounded-xl p-5 border border-emerald-500/30">
                                    <div className="flex items-center gap-2 mb-2">
                                        <div className="p-2 bg-emerald-500/20 rounded-lg">
                                            <Wallet className="h-4 w-4 text-emerald-400" />
                                        </div>
                                        <span className="text-xs font-medium text-emerald-400">YOUR PROFIT</span>
                                    </div>
                                    <p className="text-2xl font-bold text-white">{formatCurrency(earnings.summary.netProfit)}</p>
                                    <p className="text-xs text-emerald-300/70 mt-1">After commission</p>
                                </div>

                                {/* Commission Paid */}
                                <div className="bg-gradient-to-br from-rose-500/20 to-pink-500/10 rounded-xl p-5 border border-rose-500/30">
                                    <div className="flex items-center gap-2 mb-2">
                                        <div className="p-2 bg-rose-500/20 rounded-lg">
                                            <TrendingDown className="h-4 w-4 text-rose-400" />
                                        </div>
                                        <span className="text-xs font-medium text-rose-400">COMMISSION PAID</span>
                                    </div>
                                    <p className="text-2xl font-bold text-white">{formatCurrency(earnings.summary.totalCommissionPaid)}</p>
                                    <p className="text-xs text-rose-300/70 mt-1">{earnings.summary.commissionRate}% platform fee</p>
                                </div>
                                ...
                                <div className="bg-slate-800/50 rounded-xl p-5 border border-slate-700/50">
                                    <div className="flex items-center gap-2 mb-2">
                                        ...
                                        <span className="text-xs font-medium text-blue-400">SETTLED</span>
                                    </div>
                                    <p className="text-2xl font-bold text-white">{formatCurrency(earnings.settlements.totalPaid)}</p>
                                    <p className="text-xs text-gray-400 mt-1">{earnings.settlements.paidCount} settlements</p>
                                </div>
                                ...
                                <div className="bg-slate-800/50 rounded-xl p-5 border border-slate-700/50">
                                    ...
                                    <p className="text-2xl font-bold text-white">{earnings.settlements.pending}</p>
                                        ...
                                        <table className="w-full text-sm">
                                            <thead>
                                                <tr className="bg-slate-700/30">
                                                    <th className="text-left py-3 px-4 text-xs font-semibold text-gray-400 uppercase">Month</th>
                                                    <th className="text-center py-3 px-4 text-xs font-semibold text-gray-400 uppercase">Orders</th>
                                                    <th className="text-right py-3 px-4 text-xs font-semibold text-gray-400 uppercase">Revenue</th>
                                                    <th className="text-right py-3 px-4 text-xs font-semibold text-gray-400 uppercase">Commission</th>
                                                    <th className="text-right py-3 px-4 text-xs font-semibold text-gray-400 uppercase">Your Profit</th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y divide-slate-700/50">
                                                {earnings.monthlyBreakdown.map((month) => (
                                                    <tr key={month.month} className="hover:bg-slate-700/20 transition-colors">
                                                        <td className="py-3 px-4 font-medium text-white">{getMonthName(month.month)}</td>
                                                        <td className="py-3 px-4 text-center text-gray-300">{month.orders}</td>
                                                        <td className="py-3 px-4 text-right text-gray-300">{formatCurrency(month.revenue)}</td>
                                                        <td className="py-3 px-4 text-right text-rose-400">{formatCurrency(month.commission)}</td>
                                                        <td className="py-3 px-4 text-right text-emerald-400 font-semibold">{formatCurrency(month.profit)}</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
                                ...
                                <div className="flex items-start gap-3">
                                    <div className={`p-1.5 rounded-lg ${notification.type === 'warning' ? 'bg-amber-500/20' :
                                        notification.type === 'error' ? 'bg-red-500/20' :
                                            notification.type === 'success' ? 'bg-emerald-500/20' : 'bg-blue-500/20'
                                        }`}>
                                    ...
                                    <div>
                                        ...
                                        {notification.action && (
                                            <Link to={notification.action.url} className="text-xs text-violet-400 hover:underline mt-1 inline-block">
            ...
            {isApproved && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <Link to="/seller/products/add" className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50 hover:border-violet-500/30 hover:bg-slate-700/50 transition-all group text-center">
                    ...
                    <Link to="/seller/products" className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50 hover:border-violet-500/30 hover:bg-slate-700/50 transition-all group text-center">
                    ...
                    <Link to="/seller/orders" className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50 hover:border-violet-500/30 hover:bg-slate-700/50 transition-all group text-center">
                    ...
                    <Link to="/seller/finance" className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50 hover:border-violet-500/30 hover:bg-slate-700/50 transition-all group text-center">
            ...
            <div className="bg-slate-800/30 rounded-xl p-5 border border-slate-700/50">
                ...
                <a href="mailto:support@barlinafashion.com" className="text-sm text-violet-400 hover:underline">
